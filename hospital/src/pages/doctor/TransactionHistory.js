...
function TransactionHistory({ handleBack, setCurrentIndex, currenIndex }) {
    ...
    useEffect(()=>{
        const func =async()=>{            
            try {
                await axios.post(`http://${ip?.ip }:7700/patientTransactions`, {uid}).then((res)=>{
                    ////console.log(res);
  ...
  return (
    <div className='dashboard_body' style={{width:'100%', marginTop:'100px'}}>
        ...
        <div className='dashboard_body_patient_details_btns'>
            ...
            <button className={currenIndex === 4 && 'dashboard_body_patient_details_btns_'}>TRANSACTION HISTORY</button>
            <button onClick={()=>setCurrentIndex(6)}>URINE CHART</button>
            <button onClick={()=>setCurrentIndex(7)}>UTILITIES | CONSUMABLES</button>
        </div>
        <div className='payment_desk' style={{display:'flex', alignItems:'normal'}} >
