...
function DisApproved() {
  ...
  const handleBills =async(id)=>{
    setgetuid(id)
    try {
      await axios.post(`http://${ip?.ip }:7700/getDisapprovedBill`, { uid: id }).then((res)=>{
        console.log(res);
  ...
  useEffect(()=>{
    ...
    const func =async()=>{
      try {
          await axios.post(`http://${ip?.ip}:7700/getDisapprovedBillPatient`, {signal: controller.signal}).then((res)=>{
            ////console.log(res);
