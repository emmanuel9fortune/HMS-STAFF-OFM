...
function PatientDetails({setview}) {
    ...
    useEffect(()=>{
        ...
        const func=async()=>{
            try {
                await axios.post(`http://${ip?.ip }:7700/getPatientDetails`, {uid: pid || id?.uid, signal: controller.signal}).then((res)=>{
                    console.log(res);
    ...
    const handleSave =async()=>{
        try {
            await axios.post(`http://${ip?.ip}:7700/editPatientDetails`, {changes: changes, uid: pid || id?.uid}).then((res)=>{
                if(res.data.status === 'success'){
                    setreload(reload + 1)
    ...
    const handleDeposit=async()=>{
        try {
            await axios.post(`http://${ip?.ip }:7700/adddept`,{uid, deposit: dept}).then((res)=>{
                console.log(res);
    ...
    const handleRefund=async()=>{
        try {
            await axios.post(`http://${ip?.ip }:7700/refund`,{uid}).then((res)=>{     
                console.log(res);
                ...
                <div className='dashboard_body' >
                    ...
                    <div className='patient_details_' >
                        <div className='patient_details_input_field1' >
                            { 
                                status === 'admitted' || status === 'emergency'  || deposit  ?
                                    <div className='patient_details_input_field1_'>
                                        <h4>DEPOSIT</h4>
                                        <div className='deposit_' >
                                            {
                                                !deposit ?
                                                <input value={dept} type='number' onChange={(e)=> setdept(e.target.value)} placeholder='Enter Patient Deposit' />
                                                : 
                                                <h3 style={{margin:'20px'}} >{formatted}</h3>
                                            }
                                            {
                                                !deposit ?
                                                    <button onClick={handleDeposit} >SAVE</button>
                                                :
                                                    <button onClick={handleRefund} >CLEAR DEPOSIT</button>
                                            }
                                        </div>
                                    </div>
                                : null
...

                        {
                            getFamilyMembers?.length > 0 ?
                                getFamilyMembers?.map((item, i)=>(
                                    <button onClick={()=>setpid(item?._id)} style={{textTransform:'uppercase'}} key={i} >{item?.name}</button>
                                ))
                            : null
                        }
                    </div>
