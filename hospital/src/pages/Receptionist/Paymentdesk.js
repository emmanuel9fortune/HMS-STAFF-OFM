...
function Paymentdesk() {
  ...
  const handleBills =async(id, name)=>{
    setgetuid(id)
    setusername(name)
    try {
  ...
  const [dept, setdept] = useState('')
    ...
    const handleDiscount =async()=>{
      try {
        ...
        const response = await axios.post(`http://${ip?.ip}:7700/patientdiscount`, value);  
                      ...
                      const getStaff = staffs?.length > 0 ? staffs?.find((stf)=>stf?._id === getService?.staffID) : []
                        ...
                        <div key={i} style={getuid === item?._id ? {backgroundColor:'#8c8c8c8c'} : {}} className='payment_desk_cart_fields_slides'>
                          ...
                          <div style={{display:'flex', flexDirection:'column', alignItems:'center'}}>
                            <button 
                              onClick={() => {
                                handleBills(item?._id, item?.name);
                    ...
                    <div className='deposit_' >
                      <div>
                        ...
                        <div className='patient_details_input_field1_' >
                            <h3 style={{ margin:'10px 0'}}>Add Discount</h3>
                            <div style={{display:'flex', alignItems:'center'}} >
                              <input style={{padding:'5px', border:'2px solid blue'}} value={discount} onChange={(e)=> setdiscount(e.target.value)} placeholder='Enter Discount Amount' />

                              {
                                discount ?
                                <button onClick={handleDiscount} style={{padding:'12px', backgroundColor:'blue', color:'white', border:'none'}} >SAVE</button>
                                :
                                <button style={{padding:'12px', backgroundColor:'blue', color:'white', border:'none', opacity:.3}} >SAVE</button>
                              }
                            </div>
                        </div>
                        {
