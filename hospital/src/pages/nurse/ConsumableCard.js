...
import { addToConsume, editConsumeItem, removeFromConsume } from '../../features/consumables';

function ConsumableCard({item, reload, setreload}) {

    const dispatch = useDispatch()
    
    const cartItems = useSelector(state => state.Consume.items);
    ...
    const handleAddToCart = () => {
        ...
        dispatch(addToConsume(cartItem));
        setreload(reload + 1)
    };
    ...
    const handleDec = () => {
      ...
      const handleRemove = () => {
        dispatch(removeFromConsume(item?._id));
        setreload(reload + 1)
      };
  ...
  return (
      <div className='recentpatientdashcard'>
          ...
          { 
            !getitem ?
                Number(newQuantity) > Number(item?.quantity) ?
                    <button style={{width:'25%', cursor:'not-allowed', opacity:'.3'}} className='ADD_Cart_btn'>ADD TO CART</button>
                :
